...
from ..ObjectHierarchy.ItemClasses import Directions, Key, BarlinesAndMarkers, Clef, Meter, \
    Meta, Harmony, Note, Mark, Ornaments, Part
from .. import Exceptions
from ... import helpers
from ..ObjectHierarchy.TreeClasses import PieceTree
...
class MxmlParser(object):
    ...
    def __init__(self, excluded=[]):
        ...
        self.structure = {
            # HKT: added work_title and work_number
            "work-title": SetupPiece,
            "work-number": SetupPiece,
            "movement-title": SetupPiece,
            "movement-number": SetupPiece,
            "credit": SetupPiece,
            "rights": SetupPiece,
            "creator": SetupPiece,
    ...
    if content is not [] and len(tag) > 0:
        ...
        if tag[-1] == "work-title":
            ...
            if "work-title" in content:
                work_title = content["work-title"]
        # HKT: added work_number
        if tag[-1] == "work-number":
            return_val = 1
            if "work-number" in content:
                work_number = content["work-number"]
        # HKT: renamed title to movement_title
        if tag[-1] == "movement-title":
            return_val = 1
            if "movement-title" in content:
                movement_title = content["movement-title"]
        if tag[-1] == "creator":
            return_val = 1
        ...
        # HKT: added work_title, work_number, movement_number
        if tag[-1] in ["work-title", "work-number", "movement-title", "movement-number", "creator"]:
            if not hasattr(piece.GetItem(), "meta"):
                ...
                if work_title is not None:
                    piece.GetItem().meta.work_title = work_title
                if work_number is not None:
                    piece.GetItem().meta.work_number = work_number
                if movement_title is not None:
                    piece.GetItem().meta.movement_title = movement_title
                if movement_number is not None:
                    piece.GetItem().meta.movement_number = movement_number
                        ...
                        if not hasattr(piece.GetItem().meta, "copyright"):
                            ...
                            if text.replace(
                                ...
                                " ",
                                    ""):
                                piece.GetItem().meta.copyright += text
                    # # HKT: Setting work_title from credit (?)
                    # if data["handleType"] == "title":
                    #     if not hasattr(piece.GetItem().meta, "work_title"):
                    #         piece.GetItem().meta.work_title = text
                    #     else:
                    #         if text.replace(" ", "") not in piece.GetItem().meta.work_title.replace(" ", "") \
                    #                 and piece.GetItem().meta.work_title.replace(" ", "") not in text.replace(" ",""):
                    #             piece.GetItem().meta.work_title += text
